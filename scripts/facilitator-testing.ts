...
async function main() {
  const facilitatorContractAddress = "0x403a192aE6c938E2290d2e4C62338B965379f747";
  ...
  const burnContract = BurnContract.attach(facilitatorContractAddress);

  const ghoToken = await ethers.getContractAt("IGhoToken", ghoTokenAddress, signer);

  const facilitatorInfoBefore = await ghoToken.getFacilitator(facilitatorContractAddress);
  ...
  const facilitatorInfo = await ghoToken.getFacilitator(facilitatorContractAddress);
  console.log("After Minting: ", facilitatorInfo);

  const transferTx = await ghoToken.transfer(facilitatorContractAddress, burnAmount);
  ...
  const facilitatorInfoAfter = await ghoToken.getFacilitator(facilitatorContractAddress);
